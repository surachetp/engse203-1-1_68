...
function showHelp() {
  ...
  console.log('    Flags for list/search:');
  console.log('      --sort <priority|date>   - Sort results by priority or date');
  console.log('      --overdue                - Show only overdue tasks (requires due date)');
  console.log('      --tag <tag>              - Filter tasks by tag');
  ...
  try {
    switch (command) {
      case 'add':
        ...
        // parse simple flags: --due <date> --tag <tag>
        let priority = 'medium';
        let due = null;
        const tags = [];

        // determine priority if present and not a flag
        if (args[2] && !args[2].startsWith('--')) priority = args[2];
        ...
        await taskManager.addTask(args[1], priority, { due, tags });
        break;

      case 'list':
        // support flags: --sort <priority|date>, --overdue, --tag <tag>
        let filter = 'all';
        const options = {};
        if (args[1] && !args[1].startsWith('--')) filter = args[1];
      ...
      case 'search':
        ...
        // collect simple flags for search
        const searchOpts = {};
        for (let i = 2; i < args.length; i++) {
          const a = args[i];
          if (a === '--tag' && args[i + 1]) {
            searchOpts.tag = args[i + 1];
            i++;
          } else if (a === '--sort' && args[i + 1]) {
            searchOpts.sort = args[i + 1];
            i++;
          }
        }
        await taskManager.searchTasks(args[1], searchOpts);
        break;
